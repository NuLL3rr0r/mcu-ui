...
void TMSysMonSMGeneralInfo::RefreshSysResourceUsage()
{
    ...
    if ((network_stats = sg_get_network_io_stats_diff(&num_network_stats)) != NULL) {
        ...
        netTable->elementAt(num_diskio_stats + 1, 0)->addWidget(new WText("Total Network IO"));
        netTable->elementAt(num_diskio_stats + 1, 1)->addWidget(new WText(lexical_cast<string>(netTrafficTotalRx)));
        netTable->elementAt(num_diskio_stats + 1, 2)->addWidget(new WText(lexical_cast<string>(netTrafficTotalTx)));
        netTable->elementAt(num_diskio_stats + 1, 3)->addWidget(new WText(lexical_cast<string>(netTrafficTotalPacketsIn)));
        netTable->elementAt(num_diskio_stats + 1, 4)->addWidget(new WText(lexical_cast<string>(netTrafficTotalPacketsOut)));
        netTable->elementAt(num_diskio_stats + 1, 5)->addWidget(new WText(lexical_cast<string>(netTrafficTotalErrorsIn)));
        netTable->elementAt(num_diskio_stats + 1, 6)->addWidget(new WText(lexical_cast<string>(netTrafficTotalErrorsOut)));
        netTable->elementAt(num_diskio_stats + 1, 7)->addWidget(new WText(lexical_cast<string>(netTrafficTotalCollisions)));
        netTable->elementAt(num_diskio_stats + 1, 8)->addWidget(new WText("-"));
